    ...
    const loginClick = async() => {
      setLoading(true);
        ...
        if(err.length > 0) {
          ...
          let loginData = { //backend develpoper nae connect p lok
            method: "get",
            url: `admin/login`,
            params: {
            user_code : userCode,
          ...
          if (response.flag === false) {//server nae connect lr check
            ...
            if (response.data.status == "OK") {
              ...
              localStorage.setItem(`LoginProcess`, "true");//permission htain poh
              setError([]);
            } else {
